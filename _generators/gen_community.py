...
for i, e in enumerate(entries, 1):
    anchor = f"{canon}#{e['id']}"
    item = {"@type": "CreativeWork", "@id": anchor, "url": anchor, "name": e["headline"],
            "datePublished": e["date"], "image": ORIGIN + img_url(e, "1600.webp")}
    if e.get("location"):
        item["contentLocation"] = {"@type": "Place", "name": e["location"]}
...
HOME = f'''{START} generated by _generators/gen_community.py from data/community.json. Edit the data, not this block. -->
    <style>
        ...
        .cm-grid {{ display: flex; flex-wrap: wrap; justify-content: center; gap: 20px; margin: 0 auto 22px; }}
        .cm-card {{ flex: 0 1 320px; min-width: 260px; display: flex; flex-direction: column; background: var(--card); border: 1px solid var(--line); border-radius: 18px; overflow: hidden; text-align: left; color: inherit; text-decoration: none; transition: border-color .2s, transform .2s; }}
